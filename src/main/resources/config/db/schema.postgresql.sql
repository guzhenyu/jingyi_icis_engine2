...
CREATE TABLE his_patient_records (
    ...
    attending_doctor_name VARCHAR(255), -- 主治医生姓名
    -- 入科信息
    from_dept_name VARCHAR(255), -- 来源科室名称
    from_dept_id VARCHAR(255), -- 来源科室编码
    admission_status INTEGER, -- HIS状态：1-在科，3-出院
    ...
    discharge_time TIMESTAMP, -- HIS出院时间
    -- 手术信息
...
COMMENT ON COLUMN his_patient_records.admission_status IS 'HIS状态：1-在科，3-出院';
...
COMMENT ON COLUMN his_patient_records.discharge_time IS 'HIS出院时间';
...
COMMENT ON COLUMN diagnosis_history.patient_key_type IS '病人关联键类型';
COMMENT ON COLUMN diagnosis_history.his_encounter_id IS 'HIS就诊ID';
COMMENT ON COLUMN diagnosis_history.his_mrn IS '住院号';
...
COMMENT ON COLUMN device_infos.source_mode IS '数据来源模式，1-INBOUND_SERVER，2-OUTBOUND_CLIENT，3-NON_DIRECT_CONNECT';
...
COMMENT ON COLUMN device_infos.pds_ip_seq IS 'PDS realtime 目标监护仪的 ipSeq；第一版床旁 target 默认 0';
...
CREATE TABLE sepsis_and_septic_shock_bundles (
    id BIGSERIAL PRIMARY KEY,
    pid BIGINT NOT NULL,
    t0 TIMESTAMP,
    need_bundle BOOLEAN NOT NULL,
    no_bundle_reason TEXT,

    h1_lactate_initial BOOLEAN,
    h1_lactate_initial_time TIMESTAMP,
    h1_lactate_initial_note TEXT,

    h1_culture_before_abx BOOLEAN,
    h1_culture_before_abx_time TIMESTAMP,
    h1_culture_before_abx_note TEXT,

    h1_abx_broad BOOLEAN,
    h1_abx_broad_time TIMESTAMP,
    h1_abx_broad_note TEXT,

    fluid_qualified BOOLEAN,
    fluid_30mlkg BOOLEAN,
    fluid_30mlkg_time TIMESTAMP,
    fluid_30mlkg_note TEXT,

    vasopressor_qualified BOOLEAN,
    vasopressor BOOLEAN,
    vasopressor_time TIMESTAMP,
    vasopressor_note TEXT,

    relactate_qualified BOOLEAN,
    relactate BOOLEAN,
    relactate_time TIMESTAMP,
    relactate_note TEXT,

    perfusion_reassessment_details TEXT,

    modified_at TIMESTAMP NOT NULL,
    modified_by VARCHAR(255) NOT NULL
);
COMMENT ON TABLE sepsis_and_septic_shock_bundles IS '脓毒症与感染性休克集束治疗记录表';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.id IS '自增主键';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.pid IS '病人ID，对应patient_records.id';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.t0 IS '集束治疗计时起点T0的utc时间';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.need_bundle IS '是否需要集束治疗';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.no_bundle_reason IS '不需要集束治疗的理由';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.h1_lactate_initial IS 't0开始1小时内是否完成初始乳酸测定';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.h1_lactate_initial_time IS '初始乳酸测定完成的utc时间';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.h1_lactate_initial_note IS '初始乳酸测定说明';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.h1_culture_before_abx IS 't0开始1小时内是否完成抗菌药前病原学/血培养';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.h1_culture_before_abx_time IS '抗菌药前病原学/血培养完成的utc时间';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.h1_culture_before_abx_note IS '抗菌药前病原学/血培养说明';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.h1_abx_broad IS 't0开始1小时内是否完成广谱抗菌药';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.h1_abx_broad_time IS '广谱抗菌药完成的utc时间';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.h1_abx_broad_note IS '广谱抗菌药说明';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.fluid_qualified IS 't0开始3小时内是否满足晶体液补液条件';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.fluid_30mlkg IS '是否已经开始或完成晶体液补液';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.fluid_30mlkg_time IS '晶体液补液的utc时间';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.fluid_30mlkg_note IS '晶体液补液说明';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.vasopressor_qualified IS 't0开始6小时内是否满足升压药条件';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.vasopressor IS '是否已经给升压药';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.vasopressor_time IS '给升压药的utc时间';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.vasopressor_note IS '给升压药说明';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.relactate_qualified IS 't0开始6小时内是否需要复测乳酸';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.relactate IS '是否已经复测乳酸';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.relactate_time IS '复测乳酸的utc时间';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.relactate_note IS '复测乳酸说明';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.perfusion_reassessment_details IS 'PerfusionReassessmentPB的pbtxt文本';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.modified_at IS '最后修改时间';
COMMENT ON COLUMN sepsis_and_septic_shock_bundles.modified_by IS '最后修改人';
CREATE UNIQUE INDEX idx_sepsis_bundles_pid
    ON sepsis_and_septic_shock_bundles (pid);
...
-- drop table if exists sepsis_and_septic_shock_bundles;
-- drop table if exists patient_nursing_reports;
-- drop table if exists ext_url_configs;
-- drop table if exists patient_checklist_items;
-- drop table if exists patient_checklist_groups;
-- drop table if exists patient_checklist_records;
-- drop table if exists dept_checklist_items;
-- drop table if exists dept_checklist_groups;
-- drop table if exists balance_stats_shifts;
-- drop table if exists overview_params;
-- drop table if exists overview_groups;
-- drop table if exists overview_templates;
-- drop table if exists patient_critical_lis_handlings;
-- drop table if exists patient_critical_lis_results;
-- drop table if exists patient_lis_results;
-- drop table if exists patient_lis_items;
-- drop table if exists external_lis_params;
-- drop table if exists lis_params;
-- drop table if exists raw_bga_record_details;
...
-- drop table if exists padua_risk_scores;
-- drop table if exists vte_padua_scores;
-- drop table if exists vte_caprini_scores;
-- drop table if exists sofa_scores;
-- drop table if exists apache_ii_scores;
-- drop table if exists dept_doctor_score_types;
-- drop table if exists patient_diagnoses;
-- drop table if exists disease_metas;
-- drop table if exists device_data_hourly_approx;
-- drop table if exists device_data_hourly;
-- drop table if exists device_data;
-- drop table if exists patient_devices;
-- drop table if exists device_infos;
-- drop table if exists patient_bed_history;
-- drop table if exists bed_counts;
-- drop table if exists bed_configs;
-- drop table if exists ward_reports;
-- drop table if exists dragable_forms;
-- drop table if exists dragable_form_templates;
-- drop table if exists patient_shift_records;
-- drop table if exists nursing_execution_records;
-- drop table if exists nursing_orders;
-- drop table if exists nursing_order_templates;
-- drop table if exists nursing_order_template_groups;
-- drop table if exists nursing_records;
-- drop table if exists nursing_record_templates;
-- drop table if exists nursing_record_template_groups;
-- drop table if exists patient_scores;
-- drop table if exists dept_score_groups;
-- drop table if exists patient_target_daily_balances;
-- drop table if exists patient_monitoring_record_stats_daily;
-- drop table if exists patient_monitoring_time_points;
-- drop table if exists patient_monitoring_records;
-- drop table if exists patient_monitoring_group_params;
-- drop table if exists dept_monitoring_group_params;
-- drop table if exists patient_monitoring_groups;
-- drop table if exists dept_monitoring_groups;
-- drop table if exists monitoring_params_history;
-- drop table if exists dept_monitoring_params;
-- drop table if exists monitoring_params;
-- drop table if exists patient_tube_status_records;
-- drop table if exists patient_tube_attrs;
-- drop table if exists patient_tube_records;
-- drop table if exists tube_type_statuses;
-- drop table if exists tube_type_attributes;
-- drop table if exists tube_types;
-- drop table if exists medication_execution_record_stats;
-- drop table if exists medication_execution_actions;
-- drop table if exists medication_execution_records;
-- drop table if exists medication_order_groups;
...
-- drop table if exists medication_order_validity_types;
-- drop table if exists medication_execution_action_types;
-- drop table if exists medication_channels;
-- drop table if exists order_duration_types;
-- drop table if exists intake_types;
-- drop table if exists administration_route_groups;
-- drop table if exists medication_types;
-- drop table if exists readmission_history;
-- drop table if exists out_history;
-- drop table if exists surgery_history;
-- drop table if exists diagnosis_history;
-- drop table if exists patient_records;
-- drop table if exists his_patient_records;
-- drop table if exists rbac_accounts_departments_permissions;
-- drop table if exists rbac_accounts_departments_roles;
-- drop table if exists accounts_departments;
-- drop table if exists rbac_accounts_departments;
-- drop table if exists departments;
-- drop table if exists rbac_departments;
-- drop table if exists rbac_accounts_permissions;
-- drop table if exists rbac_accounts_roles;
-- drop table if exists accounts;
-- drop table if exists rbac_accounts;
-- drop table if exists rbac_roles_roles;
