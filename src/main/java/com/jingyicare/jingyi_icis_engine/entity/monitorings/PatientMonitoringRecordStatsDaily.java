...
@Entity
@Table(
    name = "patient_monitoring_record_stats_daily",
    indexes = {
        @Index(
            name = "idx_pmr_stats_daily_pid_param_code_effective_time",
            columnList = "pid, monitoring_param_code, effective_time"
        )
    }
)
public class PatientMonitoringRecordStatsDaily {
    @Id
    ...
    private LocalDateTime effectiveTime; // 本地班次日期0点（比如上海），对应的UTC时间

    @Column(name = "param_value", length = 1000)
    private String paramValue; // 将GenericValuePB序列化字符串并进行base64编码后的字符串
    ...
    private LocalDateTime modifiedAt; // 记录的时间，为班次日期对应的整点

    @Column(name = "is_deleted", nullable = false)
    private Boolean isDeleted; // 是否已删除，默认false
